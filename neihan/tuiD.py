...
#   PRIMARY KEY (`Id`)
# ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='内涵腿表';
import datetime
import time 
import requests
import json
...
def insertChatContent(create_time,digg_count,content,url,category_name,comments,maxtime):
	...
	# 插入数据  
	sql = "INSERT INTO neihanTuiTable (create_time,digg_count,content,url,category_name,comments,time_param) VALUES ( '%s', '%s', '%s','%s','%s','%s','%s')"  
	print sql
...
def getContent():
	...
	while maxtime>1512366724:
		try:
			maxtime=maxtime-2
			url='http://lf.snssdk.com/neihan/stream/category/data/v2/?tag=joke&iid=12316421155&os_version=10.1.1&os_api=18&live_sdk_version=220&channel=App%%20Store&idfa=E8131C7D-8AD5-45A0-8E6A-5F97A90CA5A9&device_platform=iphone&app_name=joke_essay&vid=20FC79DA-3103-42B3-A92E-F2D58B8DFD34&openudid=25adf6c3bb1f51523606523f4252e49e3c619921&device_type=iPhone9,1&device_id=30277977392&ac=WIFI&screen_width=750&aid=7&version_code=6.4.1&category_id=109&count=30&level=6&message_cursor=175514038&min_time=%s&mpic=1&video_cdn_first=1'%maxtime
